...
import LaporanUser from "./LaporanUser";
import { useState } from "react";
import { lineClamp } from "../../utils/helper";
function DaftarLaporanUser({
  ...
  return (
    <div className="w-1/4">
      ...
      <p className="w-full ">{lineClamp(deskripsi, 31)}</p>

      <div className="flex gap-2 w-full mt-5">
        <button
          onClick={() => isEdit(!edit)}
          disabled={isLoading}
          className="w-1/2 bg-yellow-600 p-1 rounded-lg font-bold text-white"
        >
          Edit
        </button>
        <button
          disabled={isLoading}
          onClick={() => deleteLaporan(id)}
          className="w-1/2 bg-red-600 p-1 rounded-lg font-bold text-white"
        >
          Hapus
        </button>
      </div>

      {edit && <LaporanUser isEdit={isEdit} editValues={editLaporan} />}
    </div>
  );
}
